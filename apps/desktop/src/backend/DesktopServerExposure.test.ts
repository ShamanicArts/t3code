...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(
        NodeServices.layer,
        DesktopConfig.layerTest({ SHUV2CODE_HOME: baseDir, ...env }),
      ),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { SHUV2CODE_HOME: input.baseDir, ...input.env };
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "shuv2code-desktop-server-exposure-test-",
    });
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        SHUV2CODE_DESKTOP_LAN_HOST: "10.0.0.7",
        SHUV2CODE_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
