...
>()("@shuv2code/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  "SHUV2CODE_PORT",
  "SHUV2CODE_MODE",
  "SHUV2CODE_NO_BROWSER",
  "SHUV2CODE_HOST",
  "SHUV2CODE_DESKTOP_WS_URL",
  "SHUV2CODE_DESKTOP_LAN_ACCESS",
  "SHUV2CODE_DESKTOP_LAN_HOST",
  "SHUV2CODE_DESKTOP_HTTPS_ENDPOINTS",
  "SHUV2CODE_TAILSCALE_SERVE",
  "SHUV2CODE_TAILSCALE_SERVE_PORT",
] as const;
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's SHUV2CODE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit shuv2codeHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus SHUV2CODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.shuv2code via
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "SHUV2CODE_HOME") continue;
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus SHUV2CODE_HOME; pass it
